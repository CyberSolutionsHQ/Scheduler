...
#   PLATFORM_ADMIN_USERNAME         (existing platform_admin username)
#   PLATFORM_ADMIN_PIN              (4-digit PIN)
#
...
#
# NOTE: This script defaults several PINs to "1234" for convenience. Override them in env vars
# if you are running against any non-test environment.
...
: "${PLATFORM_ADMIN_USERNAME:?Set PLATFORM_ADMIN_USERNAME}"
...
MANAGER_A_PIN="${MANAGER_A_PIN:-1234}"
MANAGER_B_PIN="${MANAGER_B_PIN:-1234}"

EMPLOYEE_1_USERNAME="${EMPLOYEE_1_USERNAME:-employee1}"
EMPLOYEE_1_PIN="${EMPLOYEE_1_PIN:-1234}"
EMPLOYEE_2_USERNAME="${EMPLOYEE_2_USERNAME:-employee2}"
EMPLOYEE_2_PIN="${EMPLOYEE_2_PIN:-1234}"
...
echo ""
echo "1) Sign in as platform_admin (company=${PLATFORM_ADMIN_COMPANY_CODE}, username=${PLATFORM_ADMIN_USERNAME})"
...
if [[ -z "${PA_TOKEN}" || -z "${PA_USER_ID}" ]]; then
  echo "Failed to sign in as platform_admin. Response:" >&2
  echo "$pa_auth" | jq '.' >&2
  exit 1
fi
...
if [[ -z "${COMPANY_A_ID}" ]]; then
  echo "create_company_with_manager failed (Company A). Response:" >&2
  echo "$company_a_resp" | jq '.' >&2
  exit 1
fi
...
if [[ -z "${COMPANY_B_ID}" ]]; then
  echo "create_company_with_manager failed (Company B). Response:" >&2
  echo "$company_b_resp" | jq '.' >&2
  exit 1
fi
...
if [[ -z "${MA_TOKEN}" || -z "${MA_USER_ID}" ]]; then
  echo "Failed to sign in as Manager A. Response:" >&2
  echo "$ma_auth" | jq '.' >&2
  exit 1
fi
...
if [[ "$(echo "$employee_1_login_resp" | jq -r '.username // empty')" != "$EMPLOYEE_1_USERNAME" ]]; then
  echo "create_employee_login failed (Employee 1). Response:" >&2
  echo "$employee_1_login_resp" | jq '.' >&2
  exit 1
fi
...
if [[ "$(echo "$employee_2_login_resp" | jq -r '.username // empty')" != "$EMPLOYEE_2_USERNAME" ]]; then
  echo "create_employee_login failed (Employee 2). Response:" >&2
  echo "$employee_2_login_resp" | jq '.' >&2
  exit 1
fi
...
if [[ -z "${E1_TOKEN}" || -z "${E1_USER_ID}" ]]; then
  echo "Failed to sign in as Employee 1. Response:" >&2
  echo "$e1_auth" | jq '.' >&2
  exit 1
fi

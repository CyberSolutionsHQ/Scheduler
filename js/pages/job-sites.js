  ...
  try {
    ...
    async function refresh() {
      ...
      if (profile.role === "platform_admin" && !companyCode) {
        listEl.innerHTML = `<div class="small">Select a company to view job sites.</div>`;
        return;
      }

      let q = supabase
        .from("job_sites")
        .select('id, "companyCode", name, address, active, "createdAt", "updatedAt"')
        .order("name");
      ...
      if (!data?.length) {
        listEl.innerHTML = `<div class="small">No job sites yet.</div>`;
        return;
      }
                ...
                .map(
                  (s) => `
                    <tr>
                      ...
                      <td>${s.address || "—"}</td>
                      <td>
                        <input type="checkbox" data-action="toggle" data-id="${s.id}" ${
                          s.active ? "checked" : ""
                        } />
                      </td>
                      <td>
    ...
    listEl.addEventListener("change", async (e) => {
      ...
      try {
        const { error } = await supabase.from("job_sites").update({ active: cb.checked }).eq("id", id);
        if (error) throw new Error(error.message);
    ...
    listEl.addEventListener("click", async (e) => {
      ...
      const ok = window.confirm("Delete this job site?");
      if (!ok) return;
      try {
    ...
    form?.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const { error } = await supabase
          .from("job_sites")
          .insert([{ companyCode: companyCodeToUse, name, address, active }]);
        if (error) throw new Error(error.message);
        toast("Job site created.", { type: "success" });
        form.reset();
        await refresh();
      } catch (err) {
